...
class AjaxActionBuilder(ActionBuilder):
    ...
    """
    # 动作类型固定为 ajax
    action_type: str = Field("ajax", description="动作类型", alias="actionType")
    ...
    # Ajax 相关属性
    api: Union[str, Dict[str, Any]] = Field(..., description="请求地址，参考 api 格式说明")
    data: Optional[Dict[str, Any]] = Field(None, description="请求数据")
    redirect: Optional[str] = Field(None, description="请求结束后跳转的路径")
    feedback: Optional[Dict[str, Any]] = Field(None, description="请求成功后弹出的 dialog 配置")
    messages: Optional[Dict[str, str]] = Field(None, description="自定义接口返回toast信息")
