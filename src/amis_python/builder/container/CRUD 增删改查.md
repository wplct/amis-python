...
注意 CRUD 所需的数据必须放 items 中，因此如果只是想显示表格类型的数据没有分页，请使用 [Table](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/table)。
...
操作
 ...
 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Trident - 3i1vg | Internet Explorer 4.0 | Win 95+ | 4 | X |  |
| 2 | Trident - 8tl10v | Internet Explorer 5.0 | Win 95+ | 5 | C |  |
| 3 | Trident - jhtttiq | Internet Explorer 5.5 | Win 95+ | 5.5 | A | 
...
删除
 ...
 |
| 4 | Trident - yh13qa | Internet Explorer 6 | Win 98+ | 6 | A | 
...
删除
 ...
 |
| 5 | Trident - 3eu43y | Internet Explorer 7 | Win XP SP2+ | 7 | A | 
...
删除
 ...
 |
| 6 | Trident - l23m3b | AOL browser (AOL desktop) | Win XP | 6 | A | 
...
删除
 ...
 |
| 7 | Gecko - 1113o9 | Firefox 1.0 | Win 98+ / OSX.2+ | 1.7 | A | 
...
删除
 ...
 |
| 8 | Gecko - pfmwco | Firefox 1.5 | Win 98+ / OSX.2+ | 1.8 | A | 
...
删除
 ...
 |
| 9 | Gecko - 8vnilh | Firefox 2.0 | Win 98+ / OSX.2+ | 1.8 | A | 
...
删除
 ...
 |
| 10 | Gecko - 95d4ho | Firefox 3.0 | Win 2k+ / OSX.3+ | 1.9 | A | 
...
如果只想保持字符串格式，可以设置`"parsePrimitiveQuery": false` 或者 `"parsePrimitiveQuery": {"enable": false}` 关闭该特性，具体效果参考[示例](https://aisuda.bce.baidu.com/amis/examples/crud/parse-primitive-query)。如果想实现字段定制化转化类型，可以使用[配置 API 请求数据](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api#%E9%85%8D%E7%BD%AE%E8%AF%B7%E6%B1%82%E6%95%B0%E6%8D%AE)，通过表达式控制接口传递的参数类型。
...
删除功能主要有三种实现：[单条操作](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E5%8D%95%E6%9D%A1%E6%93%8D%E4%BD%9C)、[批量操作](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E6%89%B9%E9%87%8F%E6%93%8D%E4%BD%9C)或者直接添加一个操作栏，在里面放个类型为 ajax 类型的按钮即可。在这个按钮里面能获得对应的行数据，而且完成后也会自动刷新这个 CRUD 列表。
...
Table 模式支持 [Table](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/table) 中的所有功能。

这个模式下会默认开启固定表头功能，如果不需要可以使用 `"affixHeader": false` 关闭。

### List 列表模式

List 模式支持 [List](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/list) 中的所有功能。

### Cards 卡片模式

Cards 模式支持 [Cards](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/cards) 中的所有功能。
...
大部分表格展示有对数据进行检索的需求，CRUD 自身支持通过配置`filter`，实现查询条件过滤表单。`filter` 配置实际上同 [Form](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/form/index) 组件，因此支持绝大部分`form`的功能。

在条件搜索区的 `Engine` 输入框中输入任意值查询会发现结果中 `ID` 为 1 - 3 的 `Rendering engine` 列因为返回值中没有对应字段值，被错误填入了与 `filter` 中相同 `name` 的字段值，这是因为表格 Cell 通过[数据链](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/datascope-and-datachain)获取到了上层数据域 `filter` 中相同字段的数据值。这种情况可以在 CRUD `columns` 对应列配置`"canAccessSuperData": false`禁止访问父级数据域（比如: `Platform`列），或者 CRUD 追加`"canAccessSuperData": false`配置禁止所有列访问父级数据域。

**请注意**：在默认没有自定义配置 api 数据映射时，提交查询条件表单，会自动将表单中的表单项值，发送给`crud`所配置的接口，然后通过后端接口，实现对数据的过滤操作，前端默认是不会进行任何的数据过滤操作

如果想前端实现过滤功能，请看[前端一次性加载](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E5%89%8D%E7%AB%AF%E4%B8%80%E6%AC%A1%E6%80%A7%E5%8A%A0%E8%BD%BD)部分。
...
除了支持 [Table 中的列配置](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/table#%E5%88%97%E9%85%8D%E7%BD%AE) 以外，crud 还支持下面这些配置，帮助更好的操作数据
...
你可以通过[数据映射](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/data-mapping)，在`api`中获取这些参数。
...
温馨提示 开启`loadDataOnce`后，搜索和过滤将交给组件处理，默认对所有字段采用模糊匹配（比如：`mi`将会匹配`amis`）。如果首次加载数据时设置了预设条件，导致接口返回的数据集合未按照此规则过滤，则可能导致切换页码后分页错误。此时有2种方案处理： 1\. 将接口返回的列表数据按照所有字段模糊匹配的规则处理 2\. 配置[`matchFunc`](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E5%8C%B9%E9%85%8D%E5%87%BD%E6%95%B0)，自定义处理过滤
...
具体效果请参考[示例](https://aisuda.bce.baidu.com/amis/examples/crud/match-func)，从`3.6.0`版本开始，`options`中支持使用`matchSorter`函数处理复杂的过滤场景，比如前缀匹配、模糊匹配等，更多详细内容推荐查看[match-sorter](https://github.com/kentcdodds/match-sorter)。
...
默认分页、搜索后，用户选择条目会被清空，配置`keepItemSelectionOnPageChange`属性后会保留用户选择，可以实现跨页面批量操作。 同时可以通过配置`maxKeepItemSelectionLength`属性限制最大勾选数
...
3.  **reload 事件动作**：使用[事件动作](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/event-action)，指定`id`为要刷新的 CRUD 组件的`id`

刷新后默认会重置当前已选行数据，即使设置了 `keepItemSelectionOnPageChange` 为 `true`，也会重置。

## 总结行

如果是默认的表格模式，还支持增加总结行，具体请参考 [table](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/table#%E6%80%BB%E7%BB%93%E8%A1%8C) 的文档。
...
实际上，你操作当前行数据，会成为弹框这层节点的父级节点，因此你可以通过 [数据链](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/datascope-and-datachain)，获取到上层，也就是点击的行的数据，具体获取方法和普通组件获取数据域中数据的方法相同，

例如上例中 Tpl 用 `${browser}` 获取 `browser` 变量，Form 中配置`"name": "engine"` 映射 `engine` 变量。

> 遇到数据字段冲突时，可以在 [弹框上通过配置数据映射](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/dialog#%E5%BC%B9%E6%A1%86%E4%B8%8E%E6%95%B0%E6%8D%AE%E6%98%A0%E5%B0%84) 解决。
...
> 本文中的例子为了不相互影响都关闭了这个功能。 另外如果需要使用接口联动，需要设置`syncLocation: false`

`syncLocation`开启后，数据域经过地址栏同步后，原始值被转化为字符串同步回数据域，但布尔值（boolean）同步后不符合预期数据结构，导致组件渲染出错。比如查询条件表单中包含[Checkbox](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/form/checkbox)组件，此时可以设置`{"trueValue": "1", "falseValue": "0"}`，将真值和假值设置为字符串格式规避。从`3.5.0`版本开始，已经支持[`parsePrimitiveQuery`](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E8%A7%A3%E6%9E%90query%E5%8E%9F%E5%A7%8B%E7%B1%BB%E5%9E%8B)，该配置默认开启。
...
配置 `itemAction` 可以实现点击某一行后进行自定义操作，支持 [action](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/action) 里的所有配置，比如弹框、刷新其它组件等。
...
| api | [API](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) |  | CRUD 用来获取列表数据的 api。 |  |
| deferApi | [API](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) |  | 当行数据中有 defer 属性时，用此接口进一步加载内容 |  |
...
| filter | [Form](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/form/index) |  | 设置过滤器，当该表单提交后，会把数据带给当前 `mode` 刷新列表。 |  |
...
| stopAutoRefreshWhen | `string` | `""` | 通过[表达式](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/expression)来配置停止刷新的条件 |  |
...
| itemDraggableOn | `boolean` |  | 用[表达式](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/expression)来配置是否可拖拽排序 |  |
| [saveOrderApi](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#saveorderapi) | [API](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) |  | 保存排序的 api。 |  |
| [quickSaveApi](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#quicksaveapi) | [API](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) |  | 快速编辑后用来批量保存的 API。 |  |
| [quickSaveItemApi](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#quicksaveitemapi) | [API](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) |  | 快速编辑配置成及时保存时使用的 API。 |  |
| bulkActions | Array<[Action](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/action)\> |  | 批量操作列表，配置后，表格可进行选中操作。 |  |
...
| matchFunc | `string` | [`CRUDMatchFunc`](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E5%8C%B9%E9%85%8D%E5%87%BD%E6%95%B0) | 自定义匹配函数, 当开启`loadDataOnce`时，会基于该函数计算的匹配结果进行过滤，主要用于处理列字段类型较为复杂或者字段值格式和后端返回不一致的场景 | `3.5.0` |
| parsePrimitiveQuery | [`ParsePrimitiveQueryOptions`](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#parseprimitivequeryoptions) | `true` | 是否开启 Query 信息转换，开启后将会对 url 中的 Query 进行转换，默认开启，默认仅转化布尔值 | `3.6.0` |

注意除了上面这些属性，CRUD 在不同模式下的属性需要参考各自的文档，比如

-   默认[Table](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/table)模式里的[列配置](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/table#%E5%88%97%E9%85%8D%E7%BD%AE%E5%B1%9E%E8%A1%A8)。
-   [Cards](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/cards) 模式。
-   [List](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/list) 模式。
...
| filterable | `boolean` | [`QuickFilterConfig`](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/crud#quickfilterconfig) | `false` | 是否可快速搜索，`options`属性为静态选项，支持设置`source`属性从接口获取选项 |  |
| quickEdit | `boolean` | [`QuickEditConfig`](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/crud#quickeditconfig) | \- | 快速编辑，一般需要配合`quickSaveApi`接口使用 |  |
| quickEditEnabledOn | `SchemaExpression` | \- | 开启快速编辑条件[表达式](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/expression) |  |
...
| source | [`Api`](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) | `string` | \- | 选项 API 接口 | `3.6.0`版本后支持上下文变量 |
...
| saveImmediately | `boolean` 或 `{api: Api}` | `false` | 是否修改后即时保存，一般需要配合`quickSaveItemApi`接口使用，也可以直接配置[`Api`](https://aisuda.bce.baidu.com/amis/zh-CN/docs/types/api) |  |
...
| size | `"xs" | "sm" | "md" | "lg"` |  | 按钮大小，参考[按钮](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/action) |
| footerBtnSize | `"xs" | "sm" | "md" | "lg"` |  | 弹窗底部按钮大小，参考[按钮](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/action) |
| level | `string` | `default` | 按钮样式，参考[按钮](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/action) |
| draggable | `boolean` | `false` | 是否可通过拖拽排序 |
...
| modalSize | `string` |  | 弹窗大小，参考[弹窗](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/dialog) |
...
当前组件会对外派发以下事件，可以通过`onEvent`来监听这些事件，并通过`actions`来配置执行的动作，在`actions`中可以通过`${事件参数名}`或`${event.data.[事件参数名]}`来获取事件产生的数据，详细查看[事件动作](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/event-action)。

| 事件名称 | 事件参数 | 说明 |
| --- | --- | --- |
| fetchInited | `responseData` 接口数据返回  
`responseStatus` 接口返回状态  
`responseMsg` 响应消息 | 远程初始化数据接口请求完成时触发 |
| quickSaveSucc | `result` 接口数据返回  
`rows: object[]` 修改了的行集合  
`rowsDiff: object[]` 与 rows 不同的地方时，对象中只有修改的部分和主键字段  
`indexes: Array<string>` 修改的行索引，如果是树形模式，下标是字符串路劲如 `0.1`  
`rowsOrigin: object[]` 原始数据 | 快速编辑完后保存成功触发 |
| quickSaveFail | `error` 错误原因 | 快速编辑完后保存失败 |
| quickSaveItemSucc | `result` 接口数据返回  
`item: object` 当亲修改的那行数据  
`modified:object` 质只包含修改的部分  
`origin: object` 原始数据 | 快速编辑单条保存成功后触发 |
| quickSaveItemFail | `error` 错误原因 | 快速编辑单条保存失败后触发 |
| saveOrderSucc | `result` 接口数据返回  
`其他` 请参考 [拖拽排序](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E6%8B%96%E6%8B%BD%E6%8E%92%E5%BA%8F) 章节说明 | 拖拽排序保存成功后触发 |
...
表格的默认列定义的事件如下，即 click、mouseenter、mouseleave。如果列定义是其他组件，则事件表就是这个组件对应的事件表，例如列定义是 Switch 组件，则可以监听 [Switch 的 change 事件](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/form/switch#%E4%BA%8B%E4%BB%B6%E8%A1%A8)。
...
当前组件对外暴露以下特性动作，其他组件可以通过指定`actionType: 动作名称`、`componentId: 该组件id`来触发这些动作，动作配置可以通过`args: {动作配置项名称: xxx}`来配置具体的参数，详细请查看[事件动作](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/event-action#%E8%A7%A6%E5%8F%91%E5%85%B6%E4%BB%96%E7%BB%84%E4%BB%B6%E7%9A%84%E5%8A%A8%E4%BD%9C)。
