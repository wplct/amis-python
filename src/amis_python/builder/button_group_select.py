...
class ButtonGroupSelectBuilder(BaseBuilder):
    ...
    # 组件名称，用于表单提交时的标识
    name: str  # 组件名称，用于表单提交时的标识
    
    # 选项列表
    options: List[dict]  # 选项列表
    
    # 其他属性
    value: Optional[Union[str, int]] = None  # 默认选中值
    class_name: Optional[str] = None  # 指定添加 button-group-select 类名
    size: Optional[Literal["xs", "sm", "md", "lg"]] = None  # 设置按钮大小
    multiple: Optional[bool] = False  # 是否支持多选
