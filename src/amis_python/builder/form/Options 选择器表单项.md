...
它派生自 [表单项](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem)，拥有表单项所有的特性。
...
你也可以配置`source`属性，利用 [数据映射](https://aisuda.bce.baidu.com/amis/docs/concepts/data-mapping)，获取当前数据链中的变量

上例中，我们给 select 组件，配置`"source": "${items}"`，获取了当前数据域中的`items`变量作为选项组。

### 远程拉取

除了可以通过数据映射获取当前数据域中的变量以外，`source`还支持配置接口，格式为 [API](https://aisuda.bce.baidu.com/amis/docs/types/api)，用于动态返回选项组。

远程拉取接口时，返回的数据结构除了需要满足 [amis 接口要求的基本数据结构](https://aisuda.bce.baidu.com/amis/docs/types/api#%E6%8E%A5%E5%8F%A3%E8%BF%94%E5%9B%9E%E6%A0%BC%E5%BC%8F-%E9%87%8D%E8%A6%81-) 以外，用`"options"`作为选项组的`key`值，如下
...
默认多选的值格式为逗号拼接 value 值，例如：`1,2,3`，如果需要改变值格式，请阅读下面 [拼接符 delimiter](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#-e6-8b-bc-e6-8e-a5-e7-ac-a6-delimiter)、[拼接值 joinValues](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#-e6-8b-bc-e6-8e-a5-e5-80-bc-joinvalues) 和 [提取多选值 extractValue](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#-e6-8f-90-e5-8f-96-e5-a4-9a-e9-80-89-e5-80-bc-extractvalue)配置项。
...
> `addControls`属性格式为表单项数组，更多细节参考 [FormItem 表单项](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem)。
...
通过 `addDialog` 来控制弹框属性，比如通过 `size` 来调大，其它属性请参考 [dialog](https://aisuda.bce.baidu.com/amis/zh-CN/components/dialog) 的属性表
...
通过 `editDialog` 来控制弹框属性，比如通过 `size` 来调大，其它属性请参考 [dialog](https://aisuda.bce.baidu.com/amis/zh-CN/components/dialog) 的属性表
...
一些选择器组件，支持配置`autoFill`，将当前已选中的选项的某个字段的值，自动填充到表单中某个表单项中，支持[数据映射](https://aisuda.bce.baidu.com/amis/docs/concepts/data-mapping)
...
除了支持 [普通表单项属性表](https://aisuda.bce.baidu.com/amis/zh-CN/components/form/formitem#%E5%B1%9E%E6%80%A7%E8%A1%A8) 中的配置以外，还支持下面一些配置

| 属性名 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| options | `Array<object>`或`Array<string>` |  | 选项组，供用户选择 |
| source | [API](https://aisuda.bce.baidu.com/amis/docs/types/api) 或 [数据映射](https://aisuda.bce.baidu.com/amis/docs/concepts/data-mapping) |  | 选项组源，可通过数据映射获取当前数据域变量、或者配置 API 对象 |
