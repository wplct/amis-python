...
class InputTextBuilder(FormItemBuilder):
    """
...
class InputPasswordBuilder(FormItemBuilder):
    """
    ...
    reveal_password: Optional[bool] = None  # 是否展示密码显/隐按钮，默认：True


class InputDatetimeBuilder(FormItemBuilder):
    """
    ...
    input_format: Optional[str] = None  # 显示格式，如"YYYY-MM-DD HH:mm:ss"
    format: Optional[str] = None  # 值格式，默认是时间戳
    placeholder: Optional[str] = None  # 占位提示文本
    shortcuts: Optional[list] = None  # 快捷选择日期，如["yesterday", "today", "tomorrow"]
    min_date: Optional[str] = None  # 限制最小日期时间
    max_date: Optional[str] = None  # 限制最大日期时间
    utc: Optional[bool] = None  # 是否保存UTC值，默认：False
    clearable: Optional[bool] = None  # 是否可清除，默认：True
    embed: Optional[bool] = None  # 是否内联，默认：False
    time_constraints: Optional[dict] = None  # 控制时间输入范围
    is_end_date: Optional[bool] = None  # 如果为true，会自动默认为23:59:59秒，默认：False
    disabled_date: Optional[str] = None  # 用字符函数来控制哪些天不可以被点选
    pop_over_container_selector: Optional[str] = None  # 弹层挂载位置选择器
    disabled: Optional[bool] = None  # 是否禁用，默认：False
    read_only: Optional[bool] = None  # 是否只读，默认：False
    required: Optional[bool] = None  # 是否必填，默认：False
